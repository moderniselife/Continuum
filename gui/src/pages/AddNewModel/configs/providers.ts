...
Select the \`GPT-4o\` model below to complete your provider configuration, but note that this will not affect the specific model you need to select when creating your Azure deployment.`,
  ...
  lemonade: {
    ...
    longDescription:
      "Lemonade provides optimized local LLM inference with support for AMD NPU, GPU, and CPU acceleration. Visit [lemonade-server.ai](https://lemonade-server.ai/) for installation and setup instructions. Once the Lemonade server is running, you can begin using Continuum with your local models.",
    icon: "lemonade.png",
    tags: [ModelProviderTags.Local, ModelProviderTags.OpenSource],
    params: {
...
After it's up and running, you can start using Continuum.`,
    icon: "llamacpp.png",
    tags: [ModelProviderTags.Local, ModelProviderTags.OpenSource],
    packages: openSourceModels,
    collectInputFor: [...completionParamsInputsConfigs],
